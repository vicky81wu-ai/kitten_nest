...
function currentScene() { return scenes[activeIndex]; }
...
function renderSceneList() {
  ...
  scenes.forEach((scene, index) => {
    ...
    item.innerHTML = `
      ...
      <span class="sceneMeta"><span class="sceneName">${scene.id}</span><span class="sceneSmall">${scene.label}</span></span>
...
function renderForm() {
  ...
  $$('.toggleGrid input').forEach(input => { input.checked = scene.effects.includes(input.value); });
...
function renderConfig() {
  ...
  configOutput.textContent = JSON.stringify({ lab: 'nest-motion-lab', activeSceneId: currentScene().id, scenes: cleanScenes }, null, 2);
}
...
function runMask(type) {
  ...
  if (['warm-mask', 'light-sweep', 'dark-fade', 'dream-ripple'].includes(type)) {
    ...
    window.setTimeout(() => { mask.className = 'transitionMask'; }, 980);
  }
}
...
function updateCurrentConfigFromForm() {
  const scene = currentScene();
  scene.id = sceneIdInput.value.trim() || scene.id;
...
function beginDrag(event) {
  ...
  dragState = { pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, originX: scene.position.x, originY: scene.position.y };
...
$('#prevScene').addEventListener('click', () => goToScene((activeIndex - 1 + scenes.length) % scenes.length, currentScene().transition));
$('#nextScene').addEventListener('click', () => goToScene((activeIndex + 1) % scenes.length, currentScene().transition));
...
$('#resetPan').addEventListener('click', () => { currentScene().position = { ...currentScene().defaultPosition }; renderAll(); });
...
$$('.ratioBtn').forEach(button => button.addEventListener('click', () => { stageWrap.className = `stageWrap ${button.dataset.ratio}`; }));
...
$('#imageUpload').addEventListener('change', (event) => {
  ...
  scenes.push({ id: `local_${idBase || 'image'}_${Date.now().toString().slice(-5)}`, label: `本地导入：${file.name}`, src: url, panMode: 'pan-xy', defaultPosition: { x: 0, y: 0 }, position: { x: 0, y: 0 }, maxMove: { x: 140, y: 140 }, transition: 'zoom-blur', effects: [] });
